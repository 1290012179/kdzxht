<template>
	<div class="article">
		...
		<div class="btns">
			...
			<el-input style="width:130px"size="mini"placeholder="请输入关键字"v-model="params.keywords"clearable></el-input>
			<el-button size='mini' @click='toAddArticle'>添加</el-button>
			<el-button size='mini' @click='batchDeleteArticles(ids)'>批量删除</el-button>
		</div>
		<!-- 按钮区结束 -->
		<!-- 数据区 -->
		<!-- {{params}} -->
		...
		<el-dialog fullscreen :title="articleDialog.title" :visible.sync="articleDialog.visible">
		  ...
		  <el-form :model="articleDialog.form">
		    ...
		    <el-form-item label="缩略图" label-width="120px">
					<el-upload
					  action="http://39.108.75.233:8099/manager/file/upload"
		...
		methods:{
			...
			batchDeleteArticles(ids){
				this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
			        ...
			        axios.post(url,{ids}).then(({data:result})=>{
			...
			// 保存或更新文章
			saveOrUpdateArticle(){
				this.articleDialog.form.sourse = this.$refs.md.d_render;
